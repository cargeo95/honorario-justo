...
from app import MESES, db, indicadores_folder, pesos
...
def resumen_semana(fecha, solo_confirmados=False):
    ...
    with db() as conn:
        dias = [dict(r) for r in conn.execute('SELECT * FROM dias WHERE fecha BETWEEN ? AND ? ORDER BY fecha',
                                              (desde, hasta))]
    ...
    entidades = [{'entidad': e, 'departamento': d, 'cargos': len(hs),
    ...
    return {
        'desde': desde, 'hasta': hasta, 'semana_iso': f'{lunes.isocalendar()[0]}-W{lunes.isocalendar()[1]:02d}',
        'dias': dias, 'publicados': sum(d['publicados'] for d in dias),
        'analizados': sum(d['analizados'] for d in dias),
        'procesos_con_dato': len({h['proceso_id'] for h in semana}), 'cargos': len(semana),
        'mediana_smlv': round(median(smlv), 2) if smlv else None,
        'bajo_2_smlv': sum(v < 2 for v in smlv),
        'hallazgos': semana, 'cortes': cortes,
...
def _linea(h):
    ...
    return (f"- {h['cargo']} ({anos}): ${pesos(h['pago_mensual_cop'])}/mes = "
...
def escribir_resumen_semana(r, root=None):
    ...
    titulo = (f"Semana del {lunes.day} de {MESES[lunes.month - 1]} al {domingo.day} de "
    ...
    L = [f'# {titulo}', '',
         ...
         if r['mediana_smlv'] is not None else '- Mediana del pago mensual: sin dato',
    ...
    if r['destacados']:
        ...
        L += ['Aún no hay suficientes cargos comparables por franja de experiencia '
    ...
    if r['con_tabla_oficial']:
        L += ['', '## De dónde sale el precio', '',
              'Procesos que citan una tabla oficial de tarifas. Es contexto, no un juicio: '
              'la tabla puede estar alta o baja.', '']
        L += [f"- {e['entidad']} ({e['departamento']}): \"{e['referencia_tarifa']}\" "
              f"(mediana {str(e['mediana_smlv']).replace('.', ',')} SMLV)" for e in r['con_tabla_oficial']]
    L += ['', '## Por departamento', '']
    L += [f"- {d}: {v['cargos']} cargos, mediana {str(v['mediana_smlv']).replace('.', ',')} SMLV"
          for d, v in sorted(r['por_departamento'].items(), key=lambda x: -x[1]['cargos'])] or ['- (sin datos)']
    L += ['', '## Referencia por franja de experiencia (todo el histórico)', '']
    L += [f"- {f}: P25 {p25:.2f} · P75 {p75:.2f} SMLV (n={n})".replace('.', ',')
          for f, (p25, p75, n) in sorted(r['cortes'].items())] or [
        f'- Aún ninguna franja con {MIN_COMPARABLES} o más cargos comparables.']
    L += ['', 'Publicar solo cifras con estado confirmado (`app.py --marcar ID confirmado`).'
          if not r['solo_confirmados'] else 'Solo cifras confirmadas.']
