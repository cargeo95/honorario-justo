...
from honorario_justo.fuentes.documentos import render_page
from honorario_justo.fuentes.secop import link

KINDS = {'objeto': '01_Entidad_y_objeto', 'experiencia': '02_Perfiles_y_experiencia', 'presupuesto': '03_Presupuesto'}
...
def allocate(root, title, case_id):
    ...
    with sqlite3.connect(root / '.indice.sqlite', timeout=30) as conn:
        conn.execute(
            'CREATE TABLE IF NOT EXISTS investigaciones (numero INTEGER PRIMARY KEY, carpeta TEXT, proceso TEXT, fecha TEXT, estado TEXT)'
        )
        ...
        folder.mkdir()
        conn.execute(
            'INSERT INTO investigaciones VALUES (?,?,?,?,?)',
...
def default_content(case):
    ...
    return {
        ...
        'review_status': 'BORRADOR_AUTOMATICO',
        'facts': [],
        'calculations': {},
    }
...
def export_research(case, source, title, content=None, results_root=None):
    ...
    root = Path(results_root or os.environ.get('SECOP_RESULTS', config.RESULTADOS)).resolve()
    ...
    try:
        ...
        for file in (source / 'text').glob('*.json'):
            ...
            text = '\n\n'.join(
                f'--- PAGINA {p["page"]} ({p.get("method", "texto")}) ---\n{p["text"]}' for p in record.get('pages', [])
            )
        ...
        for kind, name in KINDS.items():
            ...
            citations.append(
                f'- {name}: [{item["name"]}](<Documentos/{document_names[item["file"]]}>), página {item["page"]}. Recorte normalizado: {item.get("crop") or "página completa"}.'
            )
        for file in source.glob('*.json'):
            shutil.copy2(file, folder / 'Datos' / file.name)
        (folder / 'Datos/proceso_completo.json').write_text(
            json.dumps(case, ensure_ascii=False, indent=2), encoding='utf-8'
        )
        (folder / 'Datos/contenido_editorial.json').write_text(
            json.dumps(content, ensure_ascii=False, indent=2), encoding='utf-8'
        )
        files = {
            '01_LinkedIn.txt': content['linkedin'],
            '02_X.txt': content['x_post'],
        ...
        sources += '\n\n## Descargas\n\n' + '\n'.join(
            f'- {d["name"]}: {d.get("url") or "Adjunto local"}' for d in docs.values()
        )
        ...
        manifest = {
            ...
            'characters': {
                'linkedin': len(content['linkedin']),
                'x': x_length(content['x_post']),
...
if __name__ == '__main__':
    ...
    from honorario_justo.almacenamiento.base_datos import case_folder, get_case
    ...
    result = export_research(get_case(args.case_id), case_folder(args.case_id), args.title, content)
