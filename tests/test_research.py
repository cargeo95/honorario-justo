...
from research import allocate, export_research, x_length
...
def fixture_case(tmp_path):
    ...
    evidence={k:{'file':'1.pdf','name':'Invitacion.pdf','page':1} for k in ['objeto','experiencia','presupuesto']}
    case={'id':'TEST','metadata':{'entidad':'Entidad','referencia_del_proceso':'MC-1','urlproceso':{'url':'https://example.com/process'}},'analysis':{'evidence':evidence,'documents':[{'file':'1.pdf','name':'Invitacion.pdf'}]}}
    return case,source
...
def test_number_continues_existing_folders(tmp_path):
    root=tmp_path/'Resultados';root.mkdir()
