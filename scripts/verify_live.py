...
client = SecopClient()
config = {'date_from': (date.today()-timedelta(days=90)).isoformat(), 'date_to': date.today().isoformat(),
          'keywords': 'CONSULTOR,INTERVENT,ESTUDIOS,DISEÑO,ALUMBRADO', 'department': '', 'batch_size': 5}
rows = client.processes(config)
print(json.dumps([{'id':r['id_del_proceso'],'reference':r['referencia_del_proceso'],'entity':r['entidad'],
                   'date':r.get('fecha_de_publicacion_del'),'portfolio':r.get('id_del_portafolio')} for r in rows],ensure_ascii=True,indent=2),flush=True)
