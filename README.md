...
| Archivo | Qué hace |
|---|---|
| `app.py` | CLI principal: búsqueda, corte diario (`--dia`), revisión (`--marcar`), aprendizaje, tablero, imágenes |
| `secop.py` | Cliente de Datos Abiertos (SECOP II) |
| `evidence.py` | Descarga, extracción de texto, OCR y puntuación de páginas |
| `extraccion.py` | Extracción de cargo, experiencia y pago mensual |
| `aprendizaje.py` | Aprende de las revisiones dónde están las cifras y cuánto OCR cuesta cada tipo de proceso |
| `indicadores.py`, `tablero.py`, `imagenes.py` | Resúmenes por día, tablero HTML e imágenes para LinkedIn y TikTok |
| `research.py` | Exporta un caso a una carpeta autocontenida con evidencias y textos para redes |
...
Requiere Python 3.10 o superior y las dependencias de `requirements.txt`:
...
Pide "haz la busqueda de hoy" o similar. Eso ejecuta:
...
```powershell
python research.py CO1.REQ.11017639 --title "Anapoima - costos de consultoria" --content contenido.json
```
...
```powershell
python app.py --entregado CO1.REQ.11017639
```
...
```powershell
python app.py --dia                 # último día disponible en Datos Abiertos
python app.py --dia 2026-09-18      # un día específico
python app.py --dia 2026-09-18 --limite 5 --sin-ollama   # prueba rápida
```
...
```powershell
python app.py --extraer [--reprocesar] [--sin-ollama]   # re-extrae desde lo ya descargado
python app.py --marcar 27 confirmado                     # revisión humana: confirmado | descartado | sin_revisar
python app.py --tablero                                  # Indicadores/tablero.html
python app.py --exportar                                 # CSV de hallazgos en Indicadores/
```

### Aprendizaje

Cada `--marcar ID confirmado|descartado` guarda una etiqueta en la tabla `etiquetas`, con el documento, la página, los títulos y las señales de la página de donde salió la cifra (por ejemplo "costos directos de personal", "factor multiplicador", "pólizas" o "conclusiones"). `python app.py --aprendizaje` escribe en `Indicadores/aprendizaje/`:
...
### Rutina: diario, semanal y publicación

```powershell
python app.py --dia                         # cada día: se pone al día con lo último de Datos Abiertos
python app.py --marcar 27 confirmado        # revisar los hallazgos antes de publicar
python app.py --semana                      # resumen de la semana del último corte -> Indicadores/semanas/
python app.py --imagenes semana --borrador  # imágenes para revisar (con banda BORRADOR)
python app.py --imagenes semana             # imágenes finales, solo cifras confirmadas
python app.py --imagenes dia --fecha 2026-09-18
```
